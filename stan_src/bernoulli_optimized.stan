...
model {
  for (k in 1:num_collectors)
  {
    ...
    real updateA = log(rho);
    real updateB = log(1.0 - rho);
    updateA += log(rates_hi[k])*total_positive[k]/comb;
    updateA += log(1.0 - rates_hi[k])*total_negative[k]/comb;

    updateB += log(rates_lo[k])*total_positive[k]/comb;
    updateB += log(1.0 - rates_lo[k])*total_negative[k]/comb;

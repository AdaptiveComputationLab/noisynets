...
Options parse_args(int argc, char** argv) {
	...
	for(int i = 1; i < argc; i++) {
		...
		if(argument.find(indicator) != std::string::npos) {
			...
			if(argument == "--alpha_beta_file") {
				o.alpha_beta_file = argv[i+1];
			}
			...
			if(argument == "--num_sims") {
				o.num_sims = atoi(argv[i+1]);
			}
			if(argument == "--rho") {
				o.rho = atof(argv[i+1]);
			}
...
int check_args(Options opt) {
	...
	if(opt.num_sims == -1) {
		printf("Please specify num_sims (--num_sims)\n");
		result = 0;
	}
	if(opt.rho < 0.0) {
